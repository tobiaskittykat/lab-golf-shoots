...
export const SavedConceptsModal = ({
  isOpen,
  onClose,
  savedConcepts,
  onLoadConcept,
  onDeleteConcept,
}: SavedConceptsModalProps) => {
  const handleLoad = (concept: SavedConcept) => {
          ...
          {savedConcepts.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-muted-foreground/70 mt-1">
                Save concepts from Step 2 to reuse them later
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {savedConcepts.map((concept) => (
                <div
                  key={concept.id}
                  className="group flex items-start gap-4 p-4 rounded-xl bg-secondary/30 border border-border hover:border-accent/40 transition-all"
                >
                  <button
                    onClick={() => handleLoad(concept)}
                    className="flex-1 text-left"
                  >
                    <div className="font-medium text-sm text-foreground group-hover:text-accent transition-colors mb-1">
                      {concept.title}
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {(concept.presets?.productIds?.length ?? 0) > 0 && (
                        <span className="inline-flex items-center gap-1 text-xs bg-secondary text-muted-foreground px-2 py-0.5 rounded-full">
                          ...
                          {concept.presets!.productIds!.length} Product
                          {concept.presets!.productIds!.length > 1 ? "s" : ""}
                        </span>
                      )}
