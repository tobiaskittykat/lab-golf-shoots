...
export const productFocusAngleOptions = [
  ...
  { 
    ...
    prompt: 'neutral eye-level three-quarter view, shoe resting flat on surface with entire sole, rotated 30-45 degrees with toe toward bottom-right and heel toward center-left, capturing full side profile of sole, structural volume of upper, and clear view into interior footbed',
    narrative: 'the classic hero shot in a neutral, eye-level three-quarter perspective. The camera is level with the top edge of the shoe\'s upper. The shoe is rotated roughly 30 to 45 degrees away from the camera, with the toe directed toward the bottom-right corner and the heel pointing toward the center-left. The front of the shoe is physically closer to the lens than the heel, creating a natural sense of depth and scale. The shoe rests flat and stable on the surface -- it is not tilted, propped, or floating. This specific orientation ensures the full side profile of the sole is visible, the structural volume of the upper is showcased, and the concave interior footbed is clearly seen. This is the definitive e-commerce angle — authoritative, dimensional, and immediately recognizable.',
  ...
  { 
    ...
    narrative: 'a high-angle, three-quarter hero shot. The camera is positioned slightly above the shoes looking down. The foreground shoe is placed at a 45-degree diagonal with the toe pointing toward the bottom-left and heel toward the top-right, sitting flat and showcasing the interior footbed and upper details. The background shoe is positioned behind, tilted on its side so the textured outsole tread faces the viewer, angled on the opposite diagonal with the toe pointing toward the bottom-right. The two shoes form a cross-composition — the heel of the foreground shoe partially overlaps the middle section of the background shoe, creating depth.',
