...
export const RemixStep2 = ({
  ...
  // Build a short human-readable summary of what will change (NOT the full AI prompt)
  const remixSummary = useMemo(() => {
    ...
    if (selectedMark) parts.push(`Mark → ${selectedMark.name}`);
    if (parts.length === 0) parts.push('Default DF3i (no variant changes)');
    return parts.join('  ·  ');
  ...
  return (
    <div className="space-y-4 mt-8">
      ...
      {/* Section 3.5: Generation Summary */}
      <Collapsible open={openSections.prompt} onOpenChange={() => toggleSection('prompt')}>
        <div className="rounded-2xl border border-accent/30 bg-card overflow-hidden">
          <SectionHeader
            ...
            section="prompt"
            badge={`${attachedReferenceUrls.length} refs`}
          />
          <CollapsibleContent>
            <div className="px-4 pb-4 space-y-4">
              {/* Quick summary of what will change */}
              <div className="rounded-lg bg-muted/40 p-3 space-y-1">
                <p className="text-xs font-semibold text-foreground uppercase tracking-wide">What will happen</p>
                <p className="text-sm text-foreground">{remixSummary}</p>
                <p className="text-xs text-muted-foreground">
                  A detailed prompt describing the DF3i's exact shape, construction, and your variant choices will be generated automatically when you hit Generate.
                </p>
              </div>
              ...
              {attachedReferenceUrls.length > 0 && (
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-foreground uppercase tracking-wide">
                    Attached References ({attachedReferenceUrls.length})
                  </label>
                  <div className="grid grid-cols-4 sm:grid-cols-7 gap-2">
                    {attachedReferenceUrls.map((ref, idx) => (
                      ...
                      >
                        ...
                        />
                        <span className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-[9px] px-1 py-0.5 truncate text-center opacity-0 group-hover:opacity-100 transition-opacity">
                          {ref.label}
                        </span>
                      </div>
                    ))}
              ...
              {/* Source images that will be remixed */}
              {remixSourceImages.length > 0 && (
                <div className="space-y-2 border-t border-border pt-3">
                  <label className="text-xs font-semibold text-foreground uppercase tracking-wide">
                    Source Images ({remixSourceImages.length})
                  </label>
